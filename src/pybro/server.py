    ...
    if connect_target:
        ...
        for rel_path, content in files.items():
            ...
            with open(dest_path, 'w', encoding='utf-8') as f:
                f.write(content)
        print(f"[+] Codebase extracted to {state.TEMP_DIR}")
    ...
    if state.TEMP_DIR and not keep_script:
        def cleanup_temp_dir():
            if os.path.exists(state.TEMP_DIR):
                ...
                print(f"[-] Temporary codebase {state.TEMP_DIR} deleted.")
        atexit.register(cleanup_temp_dir)

    # SSL setup (identical to original)
    ssl_context = None
    cert_file_temp = None
    key_file_temp = None
    if ssl_enabled:
    ...
    try:
        with ThreadedTCPServer((bind_ip, port), EphemeralServer) as httpd:
            ...
            if state.SESSION_KEY:
                ...
                print(f"    {protocol}://<your_server_ip>:{port}/?key={state.SESSION_KEY}")
                print(f"=======================================================\n")
            else:
